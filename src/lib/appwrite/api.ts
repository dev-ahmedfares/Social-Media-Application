import { INewPost, INewUser, IUpdatedPost, IUpdateProfile } from "@/types";
...
export async function signInAccount(user: { email: string; password: string }) {
  try {
    const newAccount = await account.createEmailPasswordSession(
      user.email,
      user.password
    );
    if (!newAccount) throw Error;
...
// ============================== Update POST
export async function updatePost(post: IUpdatedPost) {
  const hasFileUploaded = post.file.length > 0;
  try {
    let image = {
    ...
    if (hasFileUploaded) {
      const fileUploaded = await uploadFile(post.file[0]);
      if (!fileUploaded) throw Error;
      const imageUrl = getFilePreview(fileUploaded.$id);

      if (!imageUrl) {
        await deleteFile(fileUploaded.$id);
        throw Error;
      }

      image = { ...image, imageUrl, imageId: fileUploaded.$id };
    }
...
// ============================== UPDATE USER PROFILE
export async function updateUserProfile(user: IUpdateProfile) {
